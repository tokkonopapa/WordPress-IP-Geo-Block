---
layout: post
title: "Confession of the trouble in 2.2.4"
...
I must make a confession that the release 2.2.4 caused a big trouble which 
stopped the sites of my plugin's users. In this article, I will report why 
this trouble happened and also how should I improve my developing process in 
order to prevent this kind of undesired accident again.
...
When the second argument `self::$wp_dirs['home']` passed to `strpos()` and it 
was an empty string, serious error occured and stopped showing the contents.
This always happened if the WordPress is installed into the document root. I 
could not predict this error because there's no mention about this behavior 
in the [PHP manual][STRPOS] which just says:
...
recovery, clean uninstall and so on).
...
type of WordPress. So as for the trouble in 2.2.4, I had never tested 
...
to the stable version if something happens <span class="emoji">
