...
In my article "[Analysis of Attack Vector against WP Plugins][AttackVector]", 
I mentioned that the basic strategy of this plugin against attacks is shown by 
the following form :
...
Well, there're [a lot of patterns][OWASP-XSS] which cause XSS. But it's hard to 
filter out these patterns from the requested URIs by general rules because they 
depend on the context which comes into view. For example, [Wordfence][Wordfence]
blocks malicious query by its [WAF][WF-WAF] when you put the word `<script>` 
into the search box.
...
blocked when you put it into the search box.

The Wordfence users doesn't need this feature but crucial for considerable 
users who doesn't install WF because of [page speed][PageSpeed].
