...
From July to September, 33 malicious requests to attempt exposing 
`wp-config.php` had been observed on my site. I analyzed all of them to 
identify if [IP Geo Block][IP-Geo-Block] can block these.

I can't find all the cause of exposure because most of them already removed 
from the WordPress repository. So I can't say the right thing with confidence, 
but the only 2 of these could be blocked even if they were from forbidden 
countries.

In this article, I should clarify how to prevent exposure of `wp-config.php` 
with IP Geo Block against such malicous requests.

<!--more-->

### <span id="sec1">Observed Requests</span> ###

Before showing the results, I'll show the description of the "**Path**":

| Symbol                                       | Path to Exploit    | Blocked |
|:---------------------------------------------|:-------------------|:--------|
| <span class="label label-success">WP</span>  | via WordPress core | Success |
| <span class="label label-danger">PD</span>   | Plugin Direct      | Fail    |
| <span class="label label-warning">N/A</span> | Not Available      | Unknown |

Those are the same meaning in [this article][Analysis]. And here's the table of
33 requests attempted to expose my `wp-config.php`. Most of them were disclosed 
recently.
...
You can find that most of them had their own download function.

### <span id="sec2">What's inside?</span> ###

Typical OMG <span class="emoji">
![emoji](https://assets-cdn.github.com/images/icons/emoji/unicode/1f631.png)
</span> code in <span class="label label-danger">PD</span> are like following :
...
### <span id="sec3">How to protect my site from this kind of OMG code?</span> ###

Should we always be careful about the plugins or themes which provide download 
functions?

If those code have a chance to load `wp-load.php` before they were excuted, 
IP Geo Block can block the attacks.

So instead of reading the PHP code in the plugins or themes which we want to 
install, please put the following directives into `.htaccess` in your plugins 
directory :
...
Here is the example of `.htaccess` in themes directory :
...
`/wp-content/themes/.../*.php`, to the [rewrite.php][WritePHP] in IP Geo Block 
and then it will be validated <span class="emoji">
