...
"[`ip-geo-api`][IP-GEO-API]" is installed. But some of the servers does not 
permit this by [PHP safe mode][SafeMode] or something similar ways mainly for 
the security reason. In this case, the previsous version fails to download the 
geo-location databases from [MaxMind][MaxMind] and [IP2Location][IP2Location] 
at the activation process. But this was not good because some users need to 
